...
import xm.lasproject.repository.db.DaoMaster;
import xm.lasproject.repository.db.DaoSession;
import xm.lasproject.repository.db.NewFriendDao;
...
public class NewFriendManager {
    ...
    private List<NewFriend> getNoVerifyNewFriend(){
        NewFriendDao dao =  openReadableDb().getNewFriendDao();
        return dao.queryBuilder().where(NewFriendDao.Properties.Status.eq(SystemVar.STATUS_VERIFY_NONE))
                .build().list();
    }
    ...
    public void updateBatchStatus(){
        ...
        if(infos!=null && infos.size()>0){
            ...
            for (int i = 0; i < size; i++) {
                ...
                msg.setStatus(SystemVar.STATUS_VERIFY_READED);
                all.add(msg);
            }
