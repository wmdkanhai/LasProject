...
import xm.lasproject.presentation.activity.BaseActivity;
 ...
 */
public abstract class BaseRecyclerAdapter1<T> extends RecyclerView.Adapter<BaseRecyclerHolder1> {
    /**
     * 默认布局
     */
     ...
     * @param datas
     */
    public BaseRecyclerAdapter1(Context context, IMutlipleItem<T> items, Collection<T> datas) {
     ...
     * @return
     */
    public BaseRecyclerAdapter1<T> bindDatas(Collection<T> datas) {
        this.lists = datas == null ? new ArrayList<T>() : new ArrayList<T>(datas);
    ...
    @Override
    public BaseRecyclerHolder1 onCreateViewHolder(ViewGroup parent, int viewType) {
        int layoutId = items.getItemLayoutId(viewType);
        LayoutInflater inflater = LayoutInflater.from(context);
        View root = inflater.inflate(layoutId, parent, false);
        return new BaseRecyclerHolder1(layoutId, root);
    }

    @Override
    public void onBindViewHolder(BaseRecyclerHolder1 holder, int position) {
     ...
     * @param item
     */
    public abstract void bindView(BaseRecyclerHolder1 holder, T item, int position);
